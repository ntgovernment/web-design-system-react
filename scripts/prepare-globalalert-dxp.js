#!/usr/bin/env node
/**
 * scripts/prepare-globalalert-dxp.js
 *
 * Copies the GlobalAlert DXP component source from
 * src/components/GlobalAlert/dxp/ to dist/components/global-alert/ without
 ...
 *
 * Run this before `npm run cmp-globalalert-dev` or
 * `npm run cmp-globalalert-deploy` whenever a full `npm run build` cannot
 * complete (e.g. dist/ files locked by running dev servers).
 *
 * Usage: node scripts/prepare-globalalert-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "GlobalAlert", "dxp");
const dest = join(root, "dist", "components", "global-alert");
...
console.log("🧩 Preparing Global Alert DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/global-alert/previews/wrapper.html
...
console.log("\n✅ dist/components/global-alert/ is ready.");
console.log("   Run:  npm run cmp-globalalert-dev   — to open the dev-ui");
console.log(
  "         npm run cmp-globalalert-deploy --dry-run  — to validate before deploy",
);
