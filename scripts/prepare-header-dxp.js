#!/usr/bin/env node
/**
 * scripts/prepare-header-dxp.js
 *
 * Copies the Header DXP component source from src/components/Header/dxp/
 * to dist/components/header/ without wiping the rest of dist/.
 *
 ...
 *
 * Run this before `npm run cmp-dev` or `npm run cmp-deploy` whenever
 * a full `npm run build` cannot complete (e.g. dist/ files locked by
 * running dev servers).
 *
 * Usage: node scripts/prepare-header-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "Header", "dxp");
const dest = join(root, "dist", "components", "header");
...
console.log("🧩 Preparing Header DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/header/previews/wrapper.html
...
console.log("\n✅ dist/components/header/ is ready.");
console.log("   Run:  npm run cmp-dev   — to open the dev-ui");
console.log("         npm run cmp-deploy --dry-run  — to validate before deploy");
