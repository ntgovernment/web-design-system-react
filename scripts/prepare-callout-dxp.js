#!/usr/bin/env node
/**
 * scripts/prepare-callout-dxp.js
 *
 * Copies the Callout DXP component source from src/components/Callout/dxp/
 * to dist/components/callout/ without wiping the rest of dist/.
 *
 ...
 *
 * Run this before `npm run cmp-callout-dev` or `npm run cmp-callout-deploy` whenever
 * a full `npm run build` cannot complete (e.g. dist/ files locked by
 * running dev servers).
 *
 * Usage: node scripts/prepare-callout-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "Callout", "dxp");
const dest = join(root, "dist", "components", "callout");
...
console.log("🧩 Preparing Callout DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/callout/previews/wrapper.html
...
console.log("\n✅ dist/components/callout/ is ready.");
console.log("   Run:  npm run cmp-callout-dev   — to open the dev-ui");
console.log("         npm run cmp-callout-deploy --dry-run  — to validate before deploy");
