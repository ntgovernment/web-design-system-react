...
try {
  ...
  if (typographyTokens.length > 0) {
    ...
    const literalCount = (literalsCSS.match(/--literal-text-transform-/g) || []).length;
    console.log(`  ✓ Generated src/themes/typography-literals.css (${literalCount} literal values)`);
  } else {
...
try {
  ...
  console.log(`  ✓ Generated src/themes/base-variables.css (${semanticVars.length} semantic variables)`);
} catch (error) {
...
try {
  ...
  const centralResult = generateCSS('NTG Central', centralData, 'central-', true);
...
try {
  ...
  const ntgBootstrapPath = join(rootDir, 'src/typography/typography-ntg.css');
  writeFileSync(ntgBootstrapPath, ntgBootstrapCSS, 'utf-8');
  console.log('  ✓ Generated src/typography/typography-ntg.css');
  ...
  const centralBootstrapPath = join(rootDir, 'src/typography/typography-central.css');
  writeFileSync(centralBootstrapPath, centralBootstrapCSS, 'utf-8');
  console.log('  ✓ Generated src/typography/typography-central.css');
} catch (error) {
...
console.log('  - src/themes/typography-literals.css (literal values for text-transform)');
...
console.log('  - src/typography/typography-ntg.css (Bootstrap overrides for NTG)');
console.log('  - src/typography/typography-central.css (Bootstrap overrides for Central)');
