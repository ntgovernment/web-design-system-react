#!/usr/bin/env node
/**
 * scripts/prepare-footer-dxp.js
 *
 * Copies the Footer DXP component source from src/components/Footer/dxp/
 * to dist/components/footer/ without wiping the rest of dist/.
 *
 ...
 *
 * Run this before `npm run cmp-footer-dev` or `npm run cmp-footer-deploy`
 * whenever a full `npm run build` cannot complete (e.g. dist/ files
 * locked by running dev servers).
 *
 * Usage: node scripts/prepare-footer-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "Footer", "dxp");
const dest = join(root, "dist", "components", "footer");
...
console.log("🧩 Preparing Footer DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/footer/previews/wrapper.html
...
console.log("\n✅ dist/components/footer/ is ready.");
console.log("   Run:  npm run cmp-footer-dev   — to open the dev-ui");
console.log(
  "         npm run cmp-footer-deploy --dry-run  — to validate before deploy",
);
