#!/usr/bin/env node
/**
 * scripts/prepare-banner-dxp.js
 *
 * Copies the Banner DXP component source from
 * src/components/Banner/dxp/ to dist/components/banner/ without
 ...
 *
 * Run this before `npm run cmp-banner-dev` or
 * `npm run cmp-banner-deploy` whenever a full `npm run build` cannot
 * complete (e.g. dist/ files locked by running dev servers).
 *
 * Usage: node scripts/prepare-banner-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "Banner", "dxp");
const dest = join(root, "dist", "components", "banner");
...
console.log("🧩 Preparing Banner DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/banner/previews/wrapper.html
...
console.log("\n✅ dist/components/banner/ is ready.");
console.log("   Run:  npm run cmp-banner-dev   — to open the dev-ui");
console.log(
  "         npm run cmp-banner-deploy:dry-run  — to validate before deploy",
);
