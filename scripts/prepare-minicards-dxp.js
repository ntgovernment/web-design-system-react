#!/usr/bin/env node
/**
 * scripts/prepare-minicards-dxp.js
 *
 * Copies the MiniCards DXP component source from src/components/MiniCards/dxp/
 * to dist/components/minicards/ without wiping the rest of dist/.
 *
 ...
 *
 * Run this before `npm run cmp-minicards-dev` or `npm run cmp-minicards-deploy` whenever
 * a full `npm run build` cannot complete (e.g. dist/ files locked by running dev servers).
 *
 * Usage: node scripts/prepare-minicards-dxp.js
 */
import {
  copyFileSync,
  existsSync,
...
const src = join(root, "src", "components", "MiniCards", "dxp");
const dest = join(root, "dist", "components", "minicards");
...
console.log("🧩 Preparing MiniCards DXP component...");
copyRecursive(src, dest);

// Inline theme CSS into dist/components/minicards/previews/wrapper.html
